...
function FloorPlan() {
  ...
  return <div className="floor-shell">
    <header className="floor-head"><div><span>Live dining room</span><h1>Floor & tables</h1><p>One glance tells the team where every guest is in their experience.</p></div><button onClick={load}><i className="bi bi-arrow-clockwise"/> Refresh now</button></header>
    <div className="floor-live"><i className="bi bi-broadcast"/> Live · refreshes every 5 seconds</div><div className="floor-legend">{statuses.map(s=><span className={'legend-state '+s} key={s}><i className={'dot '+s}/><b>{s.replaceAll('_',' ')}</b><em>{tables.filter(t=>t.status===s).length}</em></span>)}</div>
    <div className="floor-layout">{tables.map(table=>{const order=activeOrder(table);return <button className={'table-card '+table.status} key={table.id} onClick={()=>setSelected(table)}><div className="table-state">{table.status.replaceAll('_',' ')}</div><small>{table.area||'Main dining'}</small><strong>{table.number}</strong><span><i className="bi bi-people"/> {table.seats} seats</span>{order&&<em>Order #{order.id} · KSh {Number(order.total).toLocaleString()}</em>}</button>})}{!tables.length&&<div className="floor-empty">No tables yet. Add your first tables in Django Admin or through the Tables API.</div>}</div>
