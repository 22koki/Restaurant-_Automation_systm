...
function WaiterBoard() {
  ...
  const readyOrders = orders.filter((order) => (order.details || []).some((item) => item.status === 'ready'));
  ...
  return (
    <div className="waiter-shell">
      <header><span>Service pass</span><h1>Ready to run</h1><p>Food the kitchen has marked ready appears here immediately.</p></header>
      <div className="ready-grid">
        {readyOrders.map((order) => (
          <article className="ready-order" key={order.id}>
            <div className="ready-order-head"><strong>{order.table ? `Table ${order.table}` : order.order_type.replace('_', ' ')}</strong><span>Order #{order.id}</span></div>
